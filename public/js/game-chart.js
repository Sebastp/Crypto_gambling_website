...
/* WEBPACK VAR INJECTION */(function($) {function minTwoDigits(n) {
  return (n < 10 ? '0' : '') + n;
}
...
function chartGetNewX(chartElem, newTime, dataType) {
  ...
  neededTime = minTwoDigits(nedd.getHours()) + ':' + minTwoDigits(nedd.getMinutes()) + ':' + minTwoDigits(nedd.getSeconds()), latestChartX = parseInt(chartElem[0].querySelector('[chart-last-x]').getAttribute('chart-last-x'));
  ...
  if (finalX == parseInt(chartElem[0].querySelector('[chart-content]').getAttribute('chart-last-x')) && dataType == 'line') {
    noErrorTimer = 0;
    noNewData++;
...
updateTimesRatio = function updateTimesRatio(updateDone, upRatio) {
...
updateFinalProfit = function updateFinalProfit(updateDone) {
...
placeBet = function placeBet(bet_amount, bet_type, csrf_token) {
  $.ajax({
    method: 'POST',
    url: 'predict/bet',
    dataType: 'json',
